...
public class AssetLoader {
	...
	public static Sound snd[] = new Sound[200];
	public static Music mus[] = new Music[11];
	...
	public static boolean optionSave = false;
	...
	public static boolean loadedFull = false;

	int curSprite = 0;

	public static void load(boolean firstRun) {
		System.out.println("0:" + (System.currentTimeMillis() - AssetLoader.started));
		if(optionSave) {
		...
		if (firstRun) {
			...
			loadSound();		
			loadedFull = true;
			System.out.println("full:" + (System.currentTimeMillis() - AssetLoader.started));
		}
	}
	...
	public static void loadSound() {
		int i = 0;
		for(i = 0; i < 10; i++) {
			snd[100+i] = Gdx.audio.newSound(Gdx.files.local("/assets/sfx/heads/" + i + ".mp3"));
			snd[110+i] = Gdx.audio.newSound(Gdx.files.local("/assets/sfx/bodies/" + i + ".mp3"));
			
		}
		for(i = 0; i < 11; i++) {
			mus[i] = Gdx.audio.newMusic(Gdx.files.local("/assets/mus/" + i + ".mp3"));
		}
		//try {
			for(i = 0; i < 26; i++) {
				snd[i] = Gdx.audio.newSound(Gdx.files.local("/assets/sfx/" + i + ".mp3"));
			}
